...
export class UsersListComponent implements OnInit, AfterViewInit {
  ...
  deleteSelectedUsers() {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        const selectedIds = this.selection.selected.map(user => user.id).filter((id): id is number => id !== undefined);
  ...
  onDeleteMultiple() {
    this.usersService.deleteMultipleUsers(this.selectedIds).subscribe({
      next: () => {
        this.loadUsers();
        this.selectedIds = [];
        this.toastr.success('ลบผู้ใช้สำเร็จ');
      },
      error: (err) => {
        this.toastr.error(err || 'เกิดข้อผิดพลาดในการลบผู้ใช้');
      }
    });
  }
}
